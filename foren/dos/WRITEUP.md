...
Потом заметил команды, которых исходили от `10.10.0.12` к `10.10.10.3`, затем от `10.10.10.12` (не `.03`) к `172.18.0.3`
...
Это значит, что приложение использует `ptrace` так же, как и в приведенном аналоге (маскирует свои сисколлы под `ptrace`'овые) для скрытности
